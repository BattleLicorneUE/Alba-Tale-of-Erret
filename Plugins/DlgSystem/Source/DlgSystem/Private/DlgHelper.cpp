...
TMap<FName, TArray<FDlgClassAndObject>> FDlgHelper::ConvertDialogueAttendeesClassesIntoMap(const TArray<UClass*>& Classes)
{
	...
	for (UClass* Class : Classes)
	{
		...
		if (!Class->ImplementsInterface(UDlgDialogAttendee::StaticClass()))
		{
			continue;
		}
		...
		const FName AttendeeName = IDlgDialogAttendee::Execute_GetAttendeeName(Object);
		if (ObjectsMap.Contains(AttendeeName))
		{
			// Update
			ObjectsMap[AttendeeName].Add(Struct);
		}
		else
		{
			// Create
			...
			Array.Add(Struct);
			ObjectsMap.Add(AttendeeName, Array);
		}
	}
