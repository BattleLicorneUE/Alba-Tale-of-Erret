...
void SDlgDataPropertyValue::UpdateVariableNodeFromActor()
{
	...
	switch (VariableNode->GetVariableType())
	{
		...
		{
			const int32 Value = IDlgDialogAttendee::Execute_GetIntValue(Actor.Get(), VariableName);
			VariableNode->SetVariableValue(FString::FromInt(Value));
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::Float:
		{
			const float Value = IDlgDialogAttendee::Execute_GetFloatValue(Actor.Get(), VariableName);
			VariableNode->SetVariableValue(FString::SanitizeFloat(Value));
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::Bool:
		{
			const bool Value = IDlgDialogAttendee::Execute_GetBoolValue(Actor.Get(), VariableName);
			VariableNode->SetVariableValue(BoolToFString(Value));
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::FName:
		{
			const FName Value = IDlgDialogAttendee::Execute_GetNameValue(Actor.Get(), VariableName);
		...
		{
			const bool Value = IDlgDialogAttendee::Execute_CheckCondition(Actor.Get(), nullptr, VariableName);
			VariableNode->SetVariableValue(BoolToFString(Value));
			break;
		}
...
void SDlgDataTextPropertyValue::HandleTextCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
	...
	switch (VariableNode->GetVariableType())
	{
		case EDlgDataDisplayVariableTreeNodeType::Integer:
		{
			...
			IDlgDialogAttendee::Execute_ModifyIntValue(Actor.Get(), VariableName, false, Value);
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::Float:
		{
			const float Value = NewString.IsNumeric() ? FCString::Atof(*NewString) : 0.f;
			IDlgDialogAttendee::Execute_ModifyFloatValue(Actor.Get(), VariableName, false, Value);
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::Bool:
		{
			const bool Value = FStringToBool(NewString);
			IDlgDialogAttendee::Execute_ModifyBoolValue(Actor.Get(), VariableName, Value);
			break;
		}
		case EDlgDataDisplayVariableTreeNodeType::FName:
		{
			const FName Value(*NewString);
			IDlgDialogAttendee::Execute_ModifyNameValue(Actor.Get(), VariableName, Value);
			break;
		}
...
FReply SDlgDataEventPropertyValue::HandleTriggerEventClicked()
{
	...
	IDlgDialogAttendee::Execute_OnDialogueEvent(Actor.Get(), nullptr, EventName);
