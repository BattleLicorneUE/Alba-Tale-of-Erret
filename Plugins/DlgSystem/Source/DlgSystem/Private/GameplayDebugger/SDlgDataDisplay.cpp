...
// #endif
	...
	const TArray<TWeakObjectPtr<AActor>> Actors = UDlgManager::GetAllWeakActorsWithDialogueAttendeeInterface(World);

	// Build fast lookup for AttendeeNames
	// Maps from AttendeeName => Array of Dialogues that have this Attendee.
	TMap<FName, TSet<TWeakObjectPtr<const UDlgDialogue>>> AttendeeNamesDialoguesMap;
	for (const UDlgDialogue* Dialogue : Dialogues)
	{
		...
		{
			TSet<TWeakObjectPtr<const UDlgDialogue>>* ValuePtr = AttendeeNamesDialoguesMap.Find(AttendeeName);
			if (ValuePtr == nullptr)
			{
				// does not exist
				TSet<TWeakObjectPtr<const UDlgDialogue>> ValueArray{Dialogue};
				AttendeeNamesDialoguesMap.Add(AttendeeName, ValueArray);
			}
			else
			{
	...
	for (TWeakObjectPtr<AActor> Actor : Actors)
	{
		...
		// Find out the Dialogues that have the AttendeeName of this Actor.
		const FName AttendeeName = IDlgDialogAttendee::Execute_GetAttendeeName(Actor.Get());
		TSet<TWeakObjectPtr<const UDlgDialogue>> ActorDialogues;
	    TSet<TWeakObjectPtr<const UDlgDialogue>>* ActorDialoguesPtr = AttendeeNamesDialoguesMap.Find(AttendeeName);
