...
void UDlgNode_Speech::GetAssociatedAttendees(TArray<FName>& OutArray) const
{
	Super::GetAssociatedAttendees(OutArray);
	for (const FDlgTextArgument& TextArgument : TextArguments)
	{
		if (TextArgument.AttendeeName != NAME_None)
		{
			OutArray.AddUnique(TextArgument.AttendeeName);
		}
	}
}
