...
void UDlgNode::FireNodeEnterEvents(UDlgContext& Context)
{
	...
	{
		// Get Attendee from either event or parent
		UObject* Attendee = Context.GetMutableAttendee(Event.AttendeeName);
		if (!IsValid(Attendee))
		{
			Attendee = Context.GetMutableAttendee(OwnerName);
		}

		Event.Call(Context, TEXT("FireNodeEnterEvents"), Attendee);
	}
}
...
void UDlgNode::GetAssociatedAttendees(TArray<FName>& OutArray) const
{
