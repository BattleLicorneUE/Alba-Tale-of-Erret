...
void UDlgDialogue::PostLoad()
{
	...
	if (DialogueVersion < FDlgDialogueObjectVersion::AddTextFormatArguments ||
		DialogueVersion < FDlgDialogueObjectVersion::AddCustomObjectsToAttendeesData)
	{
...
void UDlgDialogue::PostEditChangeChainProperty(struct FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	// Check if the attendee UClass implements our interface
	if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(ThisClass, AttendeesClasses))
	{
		if (PropertyName == GET_MEMBER_NAME_CHECKED(FDlgAttendeeClass, AttendeeClass))
		{
			//const int32 ArrayIndex = PropertyChangedEvent.GetArrayIndex(MemberPropertyName.ToString());
			for (FDlgAttendeeClass& Attendee : AttendeesClasses)
			{
				if (!IsValid(Attendee.AttendeeClass))
				{
					continue;
				}

				if (!Attendee.AttendeeClass->ImplementsInterface(UDlgDialogAttendee::StaticClass()))
				{
					Attendee.AttendeeClass = nullptr;
				}
			}
		}
	}
...
FDlgAttendeeData& UDlgDialogue::GetAttendeeDataEntry(FName AttendeeName, FName FallbackAttendeeName, bool bCheckNone, const FString& ContextMessage)
{
	// Used to ignore some attendees
	static FDlgAttendeeData BlackHoleAttendee;

	// If the Attendee Name is not set, it adopts the Node Owner Name
	const FName& ValidAttendeeName = AttendeeName == NAME_None ? FallbackAttendeeName : AttendeeName;

	// Parent/child is not valid, simply do nothing
	if (bCheckNone && ValidAttendeeName == NAME_None)
	{
		FDlgLogger::Get().Warningf(
			TEXT("Ignoring AttendeeName = None, Context = `%s`. Either your node attendee name is None or your attendee name is None."),
			*ContextMessage
		);
		return BlackHoleAttendee;
	}

	return AttendeesData.FindOrAdd(ValidAttendeeName);
}
...
void UDlgDialogue::AddConditionsDataFromNodeEdges(const UDlgNode* Node, int32 NodeIndex)
{
	...
	const FName FallbackAttendeeName = Node->GetNodeAttendeeName();
	...
	for (const FDlgEdge& Edge : Node->GetNodeChildren())
	{
		...
		{
			if (Condition.IsAttendeeInvolved())
			{
				...
				GetAttendeeDataEntry(Condition.AttendeeName, FallbackAttendeeName, true, ContextMessage)
					.AddConditionPrimaryData(Condition);
			}
			if (Condition.IsSecondAttendeeInvolved())
			{
				...
				GetAttendeeDataEntry(Condition.OtherAttendeeName, FallbackAttendeeName, true, ContextMessage)
...
void UDlgDialogue::UpdateAndRefreshData(bool bUpdateTextsNamespacesAndKeys)
{
	...
	for (int32 NodeIndex = 0; NodeIndex < NodesNum; NodeIndex++)
	{
		...
		UDlgNode* Node = Nodes[NodeIndex];
		const FName NodeAttendeeName = Node->GetNodeAttendeeName();

		// Rebuild & Update
		RebuildAndUpdateNode(Node, *Settings, bUpdateTextsNamespacesAndKeys);

		// attendee names
		TArray<FName> Attendees;
		Node->GetAssociatedAttendees(Attendees);
		for (const FName& Attendee : Attendees)
		{
			if (!AttendeesData.Contains(Attendee))
			{
		...
		for (const FDlgCondition& Condition : Node->GetNodeEnterConditions())
		{
			if (Condition.IsAttendeeInvolved())
			{
				...
				GetAttendeeDataEntry(Condition.AttendeeName, NodeAttendeeName, true, ContextMessage)
					.AddConditionPrimaryData(Condition);
			}
			if (Condition.IsSecondAttendeeInvolved())
			{
				const FString ContextMessage = FString::Printf(TEXT("Adding secondary condition data for %s"), *NodeContext);
				GetAttendeeDataEntry(Condition.OtherAttendeeName, NodeAttendeeName, true, ContextMessage)
					.AddConditionSecondaryData(Condition);
			}
		}
		...
		for (int32 EdgeIndex = 0; EdgeIndex < NumNodeChildren; EdgeIndex++)
		{
			...
			for (const FDlgTextArgument& TextArgument : Edge.GetTextArguments())
			{
				...
				GetAttendeeDataEntry(TextArgument.AttendeeName, NodeAttendeeName, true, ContextMessage)
		...
		for (const FDlgEvent& Event : Node->GetNodeEnterEvents())
		{
			...
			GetAttendeeDataEntry(Event.AttendeeName, NodeAttendeeName, true, ContextMessage)
				.AddEventData(Event);
		}
	...
	//
	// Fill AttendeeClasses
	//
	TSet<FName> Attendees;
	GetAllAttendeeNames(Attendees);

	// 1. remove outdated entries
	for (int32 Index = AttendeesClasses.Num() - 1; Index >= 0; --Index)
	{
		const FName ExaminedName = AttendeesClasses[Index].AttendeeName;
		if (!Attendees.Contains(ExaminedName) || ExaminedName.IsNone())
		{
			AttendeesClasses.RemoveAtSwap(Index);
		}

		Attendees.Remove(ExaminedName);
	}

	// 2. add new entries
	for (const FName Attendee : Attendees)
	{
		if (Attendee != NAME_None)
		{
			AttendeesClasses.Add({ Attendee, nullptr });
		}
		else
		{
			FDlgLogger::Get().Warning(TEXT("Trying to fill AttendeesClasses, got a Attendee name = None. Ignoring!"));
		}
	}

	// 3. Set auto default attendee classes
	if (bWasLoaded && Settings->bAutoSetDefaultAttendeeClasses)
	{
		TArray<UClass*> NativeClasses;
		TArray<UClass*> BlueprintClasses;
		FDlgHelper::GetAllClassesImplementingInterface(UDlgDialogAttendee::StaticClass(), NativeClasses, BlueprintClasses);

		const TMap<FName, TArray<FDlgClassAndObject>> NativeClassesMap = FDlgHelper::ConvertDialogueAttendeesClassesIntoMap(NativeClasses);
		const TMap<FName, TArray<FDlgClassAndObject>> BlueprintClassesMap = FDlgHelper::ConvertDialogueAttendeesClassesIntoMap(BlueprintClasses);

		for (FDlgAttendeeClass& Struct : AttendeesClasses)
		{
			// Attendee Name is not set or Class is set, ignore
			if (Struct.AttendeeName == NAME_None || Struct.AttendeeClass != nullptr)
			{
				continue;
			}

			// Blueprint
			if (BlueprintClassesMap.Contains(Struct.AttendeeName))
			{
				const TArray<FDlgClassAndObject>& Array = BlueprintClassesMap.FindChecked(Struct.AttendeeName);
				if (Array.Num() == 1)
				{
					Struct.AttendeeClass = Array[0].Class;
				}
			}

			// Native last resort
			if (Struct.AttendeeClass == nullptr && NativeClassesMap.Contains(Struct.AttendeeName))
			{
				const TArray<FDlgClassAndObject>& Array = NativeClassesMap.FindChecked(Struct.AttendeeName);
				if (Array.Num() == 1)
				{
					Struct.AttendeeClass = Array[0].Class;
				}
...
void UDlgDialogue::AutoFixGraph()
{
	...
	if (!bHasEndNode && Nodes.Num() > 0)
	{
		...
		EndNode->SetNodeAttendeeName(Nodes[0]->GetNodeAttendeeName());
		Nodes.Add(EndNode);
	}
