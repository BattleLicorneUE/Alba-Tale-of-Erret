...
public:
	...
	// Converts the Classes Array that represent the Dialogue Attendees into a map where
	// Key: The attendee Name
	// Value: An array of structs that contain the class and the object for that attendee name
	static TMap<FName, TArray<FDlgClassAndObject>> ConvertDialogueAttendeesClassesIntoMap(const TArray<UClass*>& Classes);
