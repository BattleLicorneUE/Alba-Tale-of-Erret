...
public:
	// The Attendee Name (speaker) associated with this speech entry.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Dialogue|Node", Meta = (DisplayName = "Attendee Name"))
	FName Speaker;

	// Text that will appear when this node attendee name speaks to someone else.
	...
	// State of the speaker attached to the entry. Passed to the GetAttendeeIcon function.
...
public:
	...
	FName GetNodeAttendeeName() const override;
	void GetAssociatedAttendees(TArray<FName>& OutArray) const override;
