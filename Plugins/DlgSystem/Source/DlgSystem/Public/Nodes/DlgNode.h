 ...
 *  Depending on the implementation in the child class the dialogue node can contain one or more lines of one or more attendees,
...
public:
	...
	//
	// For the AttendeeName
	//

	UFUNCTION(BlueprintPure, Category = "Dialogue|Node")
	virtual FName GetNodeAttendeeName() const { return OwnerName; }

	virtual void SetNodeAttendeeName(FName InName) { OwnerName = InName; }
	...
	// Gathers associated attendees, they are only added to the array if they are not yet there
	virtual void GetAssociatedAttendees(TArray<FName>& OutArray) const;
...
protected:
	...
	// Name of a attendee (speaker) associated with this node.
	UPROPERTY(EditAnywhere, Category = "Dialogue|Node", Meta = (DisplayName = "Attendee Name"))
	FName OwnerName;
