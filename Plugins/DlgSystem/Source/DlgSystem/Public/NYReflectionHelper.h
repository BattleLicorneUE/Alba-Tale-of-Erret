...
public:
	...
	// Gathers the name of all variables in AttendeeClass (except those properties that belong to the black listed classes) from the type defined by PropertyClass
	// If the BlacklistedClasses is empty it will get the default ones from the settings
	template <typename ContainerType>
	static void GetVariableNames(
		const UClass* AttendeeClass,
		const FNYPropertyClass* PropertyClass,
		ContainerType& OutContainer,
		const TArray<UClass*>& BlacklistedClasses
	)
	{
		if (!IsValid(AttendeeClass) || !PropertyClass)
		{
			return;
		}
		...
		auto* Property = AttendeeClass->PropertyLink;
