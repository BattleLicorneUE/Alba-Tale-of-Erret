...
enum class EDlgValidateStatus : uint8
{
	...
	// Either the attendee or dialogue is invalid
	AttendeeIsNull,
	DialogueIsNull,

	// Is an instance but does not implement the UDlgDialogAttendee interface
	AttendeeDoesNotImplementInterface,

	// Is a blueprint class from the content browser and does not implement the UDlgDialogAttendee interface
	AttendeeIsABlueprintClassAndDoesNotImplementInterface,

	// The Attendee does not exist in the Dialogue
	// DialogueDoesNotContainAttendee
};
...
public:
	...
	UFUNCTION()
	void OnRep_SerializedAttendees();
	void SerializeAttendees();
	...
	*/
	...
	// Gets the Icon associated with the active node attendee name (owner name).
	UFUNCTION(BlueprintPure, Category = "Dialogue|ActiveNode")
	UTexture2D* GetActiveNodeAttendeeIcon() const;

	// Gets the Object associated with the active node attendee name (owner name).
	UFUNCTION(BlueprintPure, Category = "Dialogue|ActiveNode")
	UObject* GetActiveNodeAttendee() const;

	// Gets the active node attendee name (owner name).
	UFUNCTION(BlueprintPure, Category = "Dialogue|ActiveNode")
	FName GetActiveNodeAttendeeName() const;

	// Gets the active attendee display name
	UFUNCTION(BlueprintPure, Category = "Dialogue|ActiveNode")
	FText GetActiveNodeAttendeeDisplayName() const;

	UFUNCTION(BlueprintPure, Category = "Dialogue|Data", DisplayName = "Get Attendee")
	UObject* GetMutableAttendee(FName AttendeeName) const;
	const UObject* GetAttendee(FName AttendeeName) const;

	UFUNCTION(BlueprintPure, Category = "Dialogue|Data")
	const TMap<FName, UObject*>& GetAttendeesMap() const { return Attendees; }
	...
	UFUNCTION(BlueprintPure, Category = "Dialogue|Data")
	const TMap<FName, UObject*>& GetAttendees() const { return Attendees; }
	...
	bool Start(UDlgDialogue* InDialogue, const TMap<FName, UObject*>& InAttendees) { return StartWithContext(TEXT(""), InDialogue, InAttendees); }
	bool StartWithContext(const FString& ContextString, UDlgDialogue* InDialogue, const TMap<FName, UObject*>& InAttendees);
	...
	static bool CanBeStarted(UDlgDialogue* InDialogue, const TMap<FName, UObject*>& InAttendees);

	UFUNCTION(BlueprintPure, Category = "Dialogue|Context")
	FString GetContextString() const;

	// Checks if the Attendee object is a valid attendee for starting the Dialogue
	static EDlgValidateStatus IsValidAttendeeForDialogue(const UDlgDialogue* Dialogue, const UObject* Attendee);

	// Same as IsValidAttendeeForDialogue but this just returns a bool and logs to the output log if something is wrong
	// If bLog = true then this act exactly as IsValidAttendeeForDialogue
	static bool ValidateAttendeeForDialogue(
		const FString& ContextString,
	...
	// Same as ValidateAttendeeForDialogue but works on a Map of Attendees
	static bool ValidateAttendeesMapForDialogue(
		const FString& ContextString,
		const UDlgDialogue* Dialogue,
	...
	static bool ConvertArrayOfAttendeesToMap(
		const FString& ContextString,
		const UDlgDialogue* Dialogue,
...
protected:
	// bool StartInternal(UDlgDialogue* InDialogue, const TMap<FName, UObject*>& InAttendees, bool bLog, FString& OutErrorMessage);
	...
	void SetAttendees(const TMap<FName, UObject*>& InAttendees)
	{
		Attendees = InAttendees;
		SerializeAttendees();
	}
...
protected:
	...
	// Helper array to serialize to Attendees map for clients as well
	UPROPERTY(Replicated, ReplicatedUsing = OnRep_SerializedAttendees)
	TArray<UObject*> SerializedAttendees;

	// All object is expected to implement the IDlgDialogAttendee interface
	// the key is the return value of IDlgDialogAttendee::GetAttendeeName()
	UPROPERTY()
	TMap<FName, UObject*> Attendees;
