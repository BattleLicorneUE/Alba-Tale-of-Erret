...
)
{
	...
	// Test AttendeeName
	if (!InDlgTextArgument.AttendeeName.IsNone() &&
		InDlgTextArgument.AttendeeName.ToString().Contains(SearchFilter.SearchString))
	{
		bContainsSearchString = true;
		const FText Category = FText::Format(
			LOCTEXT("DlgTextArgumentAttendeeName", "TextArgument.AttendeeName at index = {0}"),
			FText::AsNumber(ArgumentIndex)
		);
		MakeChildTextNode(
			OutParentNode,
			FText::FromName(InDlgTextArgument.AttendeeName),
			Category,
			Category.ToString()
		);
	}
...
)
{
	...
	// Test AttendeeName
	if (!InDlgCondition.AttendeeName.IsNone() &&
		InDlgCondition.AttendeeName.ToString().Contains(SearchFilter.SearchString))
	{
		bContainsSearchString = true;
		const FText Category = FText::Format(
			LOCTEXT("DlgConditionAttendeeName", "{0}.AttendeeName at index = {1}"),
		 	 FText::FromName(ConditionMemberName), FText::AsNumber(ConditionIndex)
		);
		MakeChildTextNode(
			OutParentNode,
			FText::FromName(InDlgCondition.AttendeeName),
			Category,
			Category.ToString()
		);
	}
	...
	// Test OtherAttendeeName
	if (!InDlgCondition.OtherAttendeeName.IsNone() &&
		InDlgCondition.OtherAttendeeName.ToString().Contains(SearchFilter.SearchString))
	{
		bContainsSearchString = true;
		const FText Category = FText::Format(
			LOCTEXT("DlgConditionOtherAttendeeName", "{0}.OtherAttendeeName at index = {1}"),
			FText::FromName(ConditionMemberName), FText::AsNumber(ConditionIndex)
		);
		MakeChildTextNode(
			OutParentNode,
			FText::FromName(InDlgCondition.OtherAttendeeName),
			Category,
			Category.ToString()
		);
	}
...
)
{
	...
	// Test AttendeeName
	if (!InDlgEvent.AttendeeName.IsNone() &&
		InDlgEvent.AttendeeName.ToString().Contains(SearchFilter.SearchString))
	{
		bContainsSearchString = true;
		const FText Category = FText::Format(
			LOCTEXT("DlgEventAttendeeName", "{0}.AttendeeName at index = {1}"),
			FText::FromName(EventMemberName), FText::AsNumber(EventIndex)
		);
		MakeChildTextNode(
			OutParentNode,
			FText::FromName(InDlgEvent.AttendeeName),
			Category,
			Category.ToString()
		);
	}
...
)
{
	...
	// Test the AttendeeName
	if (Node.GetNodeAttendeeName().ToString().Contains(SearchFilter.SearchString))
	{
		bContainsSearchString = true;
		MakeChildTextNode(
			TreeGraphNode,
			FText::FromName(Node.GetNodeAttendeeName()),
			LOCTEXT("AttendeeNameKey", "Attendee Name"),
			TEXT("Attendee Name")
		);
	}
