...
void SDialogueBrowser::Construct(const FArguments& InArgs)
{
	...
	AttendeesTreeView = SNew(STreeView<TSharedPtr<FDialogueBrowserTreeNode>>)
		.ItemHeight(32)
		...
		.HeaderRow(
			...
			.DefaultLabel(LOCTEXT("Name", "Attendee Name"))
		);
...
void SDialogueBrowser::RefreshTree(bool bPreserveExpansion)
{
	...
	{
		AttendeesTreeView->GetExpandedItems(OldExpansionState);
	}

	RootTreeItem->ClearChildren();
	RootChildren.Empty();
	AttendeesProperties.Empty();
	...
	// Build fast lookup structure for attendees (the AttendeesProperties)
	...
	for (const UDlgDialogue* Dialogue : Dialogues)
	{
		...
		// Populate Attendees
		TSet<FName> AttendeesNames;
		Dialogue->GetAllAttendeeNames(AttendeesNames);
		for (const FName& AttendeeName : AttendeesNames)
		{
			TSharedPtr<FDialogueBrowserTreeAttendeeProperties>* AttendeePropsPtr = AttendeesProperties.Find(AttendeeName);
			TSharedPtr<FDialogueBrowserTreeAttendeeProperties> AttendeeProps;
			if (AttendeePropsPtr == nullptr)
			{
				// attendee does not exist, create it
				const TSet<TWeakObjectPtr<const UDlgDialogue>> SetArgument{Dialogue};
				AttendeeProps = MakeShared<FDialogueBrowserTreeAttendeeProperties>(SetArgument);
				AttendeesProperties.Add(AttendeeName, AttendeeProps);
			}
			else
			{
				// exists
				AttendeeProps = *AttendeePropsPtr;
				AttendeeProps->AddDialogue(Dialogue);
			}

			// Populate events
			TSet<FName> EventsNames;
			Dialogue->GetEvents(AttendeeName, EventsNames);
			for (const FName& EventName : EventsNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToEvent(EventName, Dialogue),
			...
			Dialogue->GetCustomEvents(AttendeeName, CustomEventsClasses);
			for (UClass* EventClass : CustomEventsClasses)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToCustomEvent(EventClass, Dialogue),
			...
			Dialogue->GetConditions(AttendeeName, ConditionNames);
			for (const FName& ConditionName : ConditionNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToCondition(ConditionName, Dialogue),
			...
			Dialogue->GetIntNames(AttendeeName, IntVariableNames);
			for (const FName& IntVariableName : IntVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToIntVariable(IntVariableName, Dialogue),
			...
			Dialogue->GetFloatNames(AttendeeName, FloatVariableNames);
			for (const FName& FloatVariableName : FloatVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToFloatVariable(FloatVariableName, Dialogue),
			...
			Dialogue->GetBoolNames(AttendeeName, BoolVariableNames);
			for (const FName& BoolVariableName : BoolVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToBoolVariable(BoolVariableName, Dialogue),
			...
			Dialogue->GetNameNames(AttendeeName, FNameVariableNames);
			for (const FName& NameVariableName : FNameVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToFNameVariable(NameVariableName, Dialogue),
			...
			Dialogue->GetClassIntNames(AttendeeName, ClassIntVariableNames);
			for (const FName& IntVariableName : ClassIntVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToClassIntVariable(IntVariableName, Dialogue),
			...
			Dialogue->GetClassFloatNames(AttendeeName, ClassFloatVariableNames);
			for (const FName& FloatVariableName : ClassFloatVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToClassFloatVariable(FloatVariableName, Dialogue),
			...
			Dialogue->GetClassBoolNames(AttendeeName, ClassBoolVariableNames);
			for (const FName& BoolVariableName : ClassBoolVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToClassBoolVariable(BoolVariableName, Dialogue),
			...
			Dialogue->GetClassNameNames(AttendeeName, ClassFNameVariableNames);
			for (const FName& NameVariableName : ClassFNameVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToClassFNameVariable(NameVariableName, Dialogue),
			...
			Dialogue->GetClassTextNames(AttendeeName, ClassFTextVariableNames);
			for (const FName& TextVariableName : ClassFTextVariableNames)
			{
				PopulateVariablePropertiesFromSearchResult(
					AttendeeProps->AddDialogueToClassFTextVariable(TextVariableName, Dialogue),
	...
	// Sort the properties
	TArray<FName> AllAttendees;
	for (const auto Elem : AttendeesProperties)
	{
		AllAttendees.Add(Elem.Key);
		TSharedPtr<FDialogueBrowserTreeAttendeeProperties> Property = Elem.Value;

		// sort
		Property->Sort();
	}

	// Sort the attendee names
	if (SelectedSortOption->IsByName())
	{
		// Sort by name
		FDlgHelper::SortDefault(AllAttendees);
	}
	else
	{
		// Sort by dialogue references
		AllAttendees.Sort([this](const FName& A, const FName& B)
		{
			return FDialogueBrowserUtilities::PredicateSortByDialoguesNumDescending(A, B, AttendeesProperties);
		});
	}

	// Build the tree
	for (const FName& Name : AllAttendees)
	{
		const TSharedPtr<FDialogueBrowserTreeNode> Attendee =
			MakeShared<FDialogueBrowserTreeCategoryAttendeeNode>(FText::FromName(Name), RootTreeItem, Name);

		BuildTreeViewItem(Attendee);
		RootTreeItem->AddChild(Attendee);
	...
	AttendeesTreeView->ClearSelection();
	// Triggers RequestTreeRefresh
	AttendeesTreeView->ClearExpandedItems();
...
void SDialogueBrowser::GenerateFilteredItems()
{
	...
	AttendeesTreeView->ClearExpandedItems(); // Triggers RequestTreeRefresh
	...
	for (const TArray<TSharedPtr<FDialogueBrowserTreeNode>>& Path : OutPaths)
	{
		...
		for (int32 PathIndex = 0; PathIndex < PathNum; PathIndex++)
		{
			...
			AttendeesTreeView->SetItemExpansion(Path[PathIndex], true);
		}
	}
...
{
	const FName AttendeeName = Item->GetParentAttendeeName();
	if (!AttendeeName.IsValid() || AttendeeName.IsNone())
	{
		return;
	}

	// Do we have the Attendee cached?
	TSharedPtr<FDialogueBrowserTreeAttendeeProperties>* AttendeePropertiesPtr = AttendeesProperties.Find(AttendeeName);
	if (AttendeePropertiesPtr == nullptr)
	{
		return;
	}

	TSharedPtr<FDialogueBrowserTreeAttendeeProperties> AttendeeProperties = *AttendeePropertiesPtr;
	if (Item->IsCategory())
	{
		switch (Item->GetCategoryType())
		{
		case EDialogueTreeNodeCategoryType::Attendee:
		{
			// Display the categories for the attendee
			const bool bHideEmptyCategories = GetDefault<UDlgSystemSettings>()->bHideEmptyDialogueBrowserCategories;
			Item->SetChildren(MakeAttendeeCategoriesChildren(Item, AttendeeProperties, bHideEmptyCategories));
			break;
		}
		case EDialogueTreeNodeCategoryType::Dialogue:
		{
			// Add the dialogues
			for (TWeakObjectPtr<const UDlgDialogue> Dialogue : AttendeeProperties->GetDialogues())
			{
				if (!Dialogue.IsValid())
				{
					continue;
				...
				DialogueItem->SetTextType(EDialogueTreeNodeTextType::AttendeeDialogue);
		...
		case EDialogueTreeNodeCategoryType::Event:
		{
			...
			for (const auto& Pair : AttendeeProperties->GetEvents())
			{
				...
				EventItem->SetTextType(EDialogueTreeNodeTextType::AttendeeEvent);
				Item->AddChild(EventItem);
			}

			// Display the custom events for this category
			for (const auto& Pair : AttendeeProperties->GetCustomEvents())
			{
				...
				CustomEventItem->SetTextType(EDialogueTreeNodeTextType::AttendeeCustomEvent);
		...
		{
			for (const auto& Pair : AttendeeProperties->GetConditions())
			{
				const TSharedPtr<FDialogueBrowserTreeNode> ConditionItem =
					MakeShared<FDialogueBrowserTreeVariableNode>(FText::FromName(Pair.Key), Item, Pair.Key);
				ConditionItem->SetTextType(EDialogueTreeNodeTextType::AttendeeCondition);
		...
		{
			// Only display the categories if the Attendee has at least one variable.
			if (AttendeeProperties->HasDialogueValues())
			{
				const bool bHideEmptyCategories = GetDefault<UDlgSystemSettings>()->bHideEmptyDialogueBrowserCategories;
				Item->SetChildren(MakeVariableCategoriesChildren(Item, AttendeeProperties, bHideEmptyCategories));
			}
			break;
		}
		case EDialogueTreeNodeCategoryType::VariableInt:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetIntegers(),
				EDialogueTreeNodeTextType::AttendeeVariableInt
			);
			break;
		case EDialogueTreeNodeCategoryType::VariableFloat:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetFloats(),
				EDialogueTreeNodeTextType::AttendeeVariableFloat
			);
			break;
		case EDialogueTreeNodeCategoryType::VariableBool:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetBools(),
				EDialogueTreeNodeTextType::AttendeeVariableBool
			);
			break;
		case EDialogueTreeNodeCategoryType::VariableFName:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetFNames(),
				EDialogueTreeNodeTextType::AttendeeVariableFName
			);
			break;

		case EDialogueTreeNodeCategoryType::ClassVariable:
		{
			// Only display the categories if the Attendee has at least one class variable.
			if (AttendeeProperties->HasClassVariables())
			{
				const bool bHideEmptyCategories = GetDefault<UDlgSystemSettings>()->bHideEmptyDialogueBrowserCategories;
				Item->SetChildren(MakeClassVariableCategoriesChildren(Item, AttendeeProperties, bHideEmptyCategories));
			}
			break;
		}
		case EDialogueTreeNodeCategoryType::ClassVariableInt:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetClassIntegers(),
				EDialogueTreeNodeTextType::AttendeeClassVariableInt
			);
			break;
		case EDialogueTreeNodeCategoryType::ClassVariableFloat:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetClassFloats(),
				EDialogueTreeNodeTextType::AttendeeClassVariableFloat
			);
			break;
		case EDialogueTreeNodeCategoryType::ClassVariableBool:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetClassBools(),
				EDialogueTreeNodeTextType::AttendeeClassVariableBool
			);
			break;
		case EDialogueTreeNodeCategoryType::ClassVariableFName:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetClassFNames(),
				EDialogueTreeNodeTextType::AttendeeClassVariableFName
			);
			break;
		case EDialogueTreeNodeCategoryType::ClassVariableFText:
			AddVariableChildrenToItem(
				Item,
				AttendeeProperties->GetClassFTexts(),
				EDialogueTreeNodeTextType::AttendeeClassVariableFText
			);
			break;
	...
	else if (Item->IsText())
	{
		...
		{
		case EDialogueTreeNodeTextType::AttendeeEvent:
			// List the dialogues that contain this event for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetEvents().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::EventDialogue
			);
			break;

		case EDialogueTreeNodeTextType::AttendeeCustomEvent:
			// List the dialogues that contain this event for this custom event attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetCustomEvents().Find(Item->GetParentClass()),
				EDialogueTreeNodeTextType::CustomEventDialogue
			);
			break;

		case EDialogueTreeNodeTextType::AttendeeCondition:
			// List the dialogues that contain this condition for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetConditions().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::ConditionDialogue
			);
			break;

		case EDialogueTreeNodeTextType::AttendeeVariableInt:
			// List the dialogues that contain this int variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetIntegers().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::IntVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeVariableFloat:
			// List the dialogues that contain this float variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetFloats().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::FloatVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeVariableBool:
			// List the dialogues that contain this bool variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetBools().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::BoolVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeVariableFName:
			// List the dialogues that contain this Fname variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetFNames().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::FNameVariableDialogue
			);
			break;

		case EDialogueTreeNodeTextType::AttendeeClassVariableInt:
			// List the dialogues that contain this UClass int variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetClassIntegers().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::IntClassVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeClassVariableFloat:
			// List the dialogues that contain this UClass float variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetClassFloats().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::FloatClassVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeClassVariableBool:
			// List the dialogues that contain this UClass bool variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetClassBools().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::BoolClassVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeClassVariableFName:
			// List the dialogues that contain this UClass Fname variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetClassFNames().Find(Item->GetParentVariableName()),
				EDialogueTreeNodeTextType::FNameClassVariableDialogue
			);
			break;
		case EDialogueTreeNodeTextType::AttendeeClassVariableFText:
			// List the dialogues that contain this UClass FText variable for this attendee
			AddDialogueChildrenToItemFromProperty(
				Item,
				AttendeeProperties->GetClassFTexts().Find(Item->GetParentVariableName()),
...
)
{
	...
	{
		if (InItem->GetCategoryType() == EDialogueTreeNodeCategoryType::Attendee)
		{
			int32 DialogueReferences = 0;
			TSharedPtr<FDialogueBrowserTreeAttendeeProperties>* AttendeePropertiesPtr =
				AttendeesProperties.Find(InItem->GetParentAttendeeName());
			if (AttendeePropertiesPtr)
			{
				DialogueReferences = (*AttendeePropertiesPtr)->GetDialogues().Num();
			}
...
void SDialogueBrowser::HandleDoubleClick(TSharedPtr<FDialogueBrowserTreeNode> InItem)
{
	...
	if (InItem->HasChildren())
	{
		AttendeesTreeView->SetItemExpansion(InItem, !AttendeesTreeView->IsItemExpanded(InItem));
	}
}
...
void SDialogueBrowser::HandleSetExpansionRecursive(TSharedPtr<FDialogueBrowserTreeNode> InItem, bool bInIsItemExpanded)
{
	...
	{
		AttendeesTreeView->SetItemExpansion(InItem, bInIsItemExpanded);
...
TArray<TSharedPtr<FDialogueBrowserTreeNode>> SDialogueBrowser::MakeAttendeeCategoriesChildren(
	const TSharedPtr<FDialogueBrowserTreeNode>& Parent,
	const TSharedPtr<FDialogueBrowserTreeAttendeeProperties>& AttendeeProperties,
	bool bHideEmptyCategories
) const
{
	TArray<TSharedPtr<FDialogueBrowserTreeNode>> Categories;
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasDialogues()))
	{
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasEvents()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasConditions()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasDialogueValues()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassVariables()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
...
{
	TArray<TSharedPtr<FDialogueBrowserTreeNode>> Categories;
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasIntegers()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasFloats()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasBools()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasFNames()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
...
) const
{
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassIntegers()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
			FText::FromString(TEXT("Integers")),
			Parent,
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassFloats()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
			FText::FromString(TEXT("Floats")),
			Parent,
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassBools()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
			FText::FromString(TEXT("Bools")),
			Parent,
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassFNames()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
			FText::FromString(TEXT("FNames")),
			Parent,
	...
	if (!bHideEmptyCategories || (bHideEmptyCategories && AttendeeProperties->HasClassFTexts()))
	{
		TSharedPtr<FDialogueBrowserTreeNode> Category = MakeShared<FDialogueBrowserTreeCategoryNode>(
