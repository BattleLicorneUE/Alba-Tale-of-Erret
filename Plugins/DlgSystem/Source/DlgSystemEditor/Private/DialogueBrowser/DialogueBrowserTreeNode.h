...
enum class EDialogueTreeNodeTextType : uint8
{
	...
	AttendeeCondition,
	AttendeeVariableInt,
	AttendeeVariableFloat,
	AttendeeVariableBool,
	AttendeeVariableFName,
	AttendeeClassVariableInt,
	AttendeeClassVariableFloat,
	AttendeeClassVariableBool,
	AttendeeClassVariableFName,
	AttendeeClassVariableFText,
...
public:
	...
	/** Gets the Attendee Name that this Node belongs to. This must always return a valid value. */
	virtual FName GetParentAttendeeName() const;
	...
	{
		return IsText() &&
			  (TextType == EDialogueTreeNodeTextType::AttendeeDialogue
	...
	{
		return IsText() && TextType == EDialogueTreeNodeTextType::AttendeeEvent;
	}
	bool IsCustomEventText() const
	{
		return IsText() && TextType == EDialogueTreeNodeTextType::AttendeeCustomEvent;
	}
	bool IsConditionText() const
	{
		return IsText() && (TextType == EDialogueTreeNodeTextType::AttendeeCondition);
	}
	...
	virtual bool IsEqual(const Self& Other)
	{
		return TextType == Other.GetTextType() &&
			...
			GetParentAttendeeName() == Other.GetParentAttendeeName() &&
...
// Node results that represents the Attendee Name.
class FDialogueBrowserTreeAttendeeNode : public FDialogueBrowserTreeNode
{
	typedef FDialogueBrowserTreeAttendeeNode Self;
	typedef FDialogueBrowserTreeNode Super;
public:
	FDialogueBrowserTreeAttendeeNode(
		const FText& InDisplayText,
		const TSharedPtr<FDialogueBrowserTreeNode>& InParent,
		FName InAttendeeName
	);

	// AttendeeName:
	FName GetParentAttendeeName() const override;

protected:
	/** The Attendee Name it represents. */
...
// Similar to the FDialogueBrowserTreeAttendeeNode only this is a Category
class FDialogueBrowserTreeCategoryAttendeeNode : public FDialogueBrowserTreeAttendeeNode
{
	typedef FDialogueBrowserTreeAttendeeNode Super;
public:
	FDialogueBrowserTreeCategoryAttendeeNode(
		const FText& InDisplayText,
		const TSharedPtr<FDialogueBrowserTreeNode>& InParent,
		FName InAttendeeName
	);
