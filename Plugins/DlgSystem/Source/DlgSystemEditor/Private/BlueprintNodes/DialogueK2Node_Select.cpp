...
bool UDialogueK2Node_Select::RefreshPinNames()
{
	...
	const FName AttendeeName = FDialogueBlueprintUtilities::GetAttendeeNameFromNode(this);
	if (AttendeeName == NAME_None && VariableType != EDlgVariableType::SpeakerState)
	{
		return false;
	}
	...
	switch (VariableType)
	{
		case EDlgVariableType::Float:
			UDlgManager::GetAllDialoguesFloatNames(AttendeeName, NewPinNames);
			break;

		case EDlgVariableType::Int:
			UDlgManager::GetAllDialoguesIntNames(AttendeeName, NewPinNames);
			break;

		case EDlgVariableType::Name:
			UDlgManager::GetAllDialoguesNameNames(AttendeeName, NewPinNames);
			break;
