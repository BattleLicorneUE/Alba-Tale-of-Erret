	...
	FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	AttendeeNamePropertyHandle = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDlgCondition, AttendeeName));
	OtherAttendeeNamePropertyHandle = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDlgCondition, OtherAttendeeName));
	...
	check(OtherAttendeeNamePropertyHandle.IsValid());
...
)
{
	...
	// AttendeeName
	{
		...
		.SetVisibility(CREATE_VISIBILITY_CALLBACK(&Self::GetAttendeeNameVisibility))
		.Update();
	}
	...
	// OtherAttendeeName
	{
		...
		AttendeeNamePropertyRow = MakeShared<FDialogueTextPropertyPickList_CustomRowHelper>(DetailWidgetRow, OtherAttendeeNamePropertyHandle);
		AttendeeNamePropertyRow->SetTextPropertyPickListWidget(
			SNew(SDialogueTextPropertyPickList)
			.IsEnabled(InStructPropertyHandle->IsEditable())
			.AvailableSuggestions(this, &Self::GetAllDialoguesAttendeeNames)
		...
		.SetVisibility(CREATE_VISIBILITY_CALLBACK(&Self::GetOtherAttendeeNameAndVariableVisibility))
		.Update();
	}
...
void FDialogueCondition_Details::OnConditionTypeChanged(bool bForceRefresh)
{
	...
	case EDlgConditionType::EventCall:
		...
		CallBackNameToolTip = LOCTEXT("ConditionEvent_CallBackNameToolTip", "Name parameter of the event call the attendee gets");
...
TArray<FName> FDialogueCondition_Details::GetCallbackNamesForAttendee(bool bCurrentOnly, bool bOtherValue) const
{
	TArray<FName> Suggestions;
	TSet<FName> SuggestionSet;
	const TSharedPtr<IPropertyHandle>& AttendeeHandle = bOtherValue ? OtherAttendeeNamePropertyHandle : AttendeeNamePropertyHandle;
	const FName AttendeeName = FDialogueDetailsPanelUtils::GetAttendeeNameFromPropertyHandle(AttendeeHandle.ToSharedRef());
	...
	case EDlgConditionType::BoolCall:
		...
		else
		{
			...
			{
				Dialogue->GetBoolNames(AttendeeName, SuggestionSet);
	...
	case EDlgConditionType::FloatCall:
		...
		else
		{
			if (bCurrentOnly && Dialogue)
			{
				Dialogue->GetFloatNames(AttendeeName, SuggestionSet);
			}
			else
			{
	...
	case EDlgConditionType::IntCall:
		...
		else
		{
			if (bCurrentOnly && Dialogue)
			{
				Dialogue->GetIntNames(AttendeeName, SuggestionSet);
			}
			else
			{
	...
	case EDlgConditionType::ClassNameVariable:
		...
		else
		{
			if (bCurrentOnly && Dialogue)
			{
				Dialogue->GetNameNames(AttendeeName, SuggestionSet);
			}
			else
			{
	...
	default:
		if (bCurrentOnly && Dialogue)
		{
			Dialogue->GetConditions(AttendeeName, SuggestionSet);
		}
		else
		{
			UDlgManager::GetAllDialoguesConditionNames(AttendeeName, Suggestions);
		}
		break;
	}
