...
private:
	...
	EVisibility GetAttendeeNameVisibility() const
	{
	...
	EVisibility GetOtherAttendeeNameAndVariableVisibility() const
	{
		return ConditionType != EDlgConditionType::WasNodeVisited
			&& ConditionType != EDlgConditionType::HasSatisfiedChild
	...
	{
		return GetCallbackNamesForAttendee(false, false);
	}
	...
	{
		return GetCallbackNamesForAttendee(true, false);
	}

	TArray<FName> GetAllDialoguesOtherVariableNames() const
	{
		return GetCallbackNamesForAttendee(false, true);
	}

	TArray<FName> GetCurrentDialogueOtherVariableNames() const
	{
		return GetCallbackNamesForAttendee(true, true);
	}

	TArray<FName> GetCallbackNamesForAttendee(bool bCurrentOnly, bool bOtherValue) const;

	// Gets the AttendeeNames from all Dialogues.
	TArray<FName> GetAllDialoguesAttendeeNames() const
	{
		TArray<FName> OutArray;
		UDlgManager::GetAllDialoguesAttendeeNames(OutArray);
		return OutArray;
	}

	// Gets the current Dialogue Attendee Names.
	TArray<FName> GetCurrentDialogueAttendeeNames() const
	{
		return FDialogueDetailsPanelUtils::GetDialogueSortedAttendeeNames(Dialogue);
	}
...
private:
	...
	TSharedPtr<IPropertyHandle> AttendeeNamePropertyHandle;
	TSharedPtr<IPropertyHandle> OtherAttendeeNamePropertyHandle;
	...
	TSharedPtr<FDialogueTextPropertyPickList_CustomRowHelper> AttendeeNamePropertyRow;
	...
	TSharedPtr<FDialogueTextPropertyPickList_CustomRowHelper> OtherAttendeeNamePropertyRow;
