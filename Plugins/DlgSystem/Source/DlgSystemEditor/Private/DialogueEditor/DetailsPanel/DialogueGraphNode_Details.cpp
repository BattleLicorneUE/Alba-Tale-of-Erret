...
void FDialogueGraphNode_Details::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (!bIsRootNode)
	{
		...
		// OwnerName
		{
			const TSharedPtr<IPropertyHandle> AttendeeNamePropertyHandle =
				PropertyDialogueNode->GetChildHandle(UDlgNode::GetMemberNameOwnerName());
			FDetailWidgetRow* DetailWidgetRow = &BaseDataCategory.AddCustomRow(LOCTEXT("AttendeeNameSearcKey", "Attendee Name"));
			...
			AttendeeNamePropertyRow->SetTextPropertyPickListWidget(
				...
				.OnTextCommitted(this, &Self::HandleAttendeeTextCommitted)
