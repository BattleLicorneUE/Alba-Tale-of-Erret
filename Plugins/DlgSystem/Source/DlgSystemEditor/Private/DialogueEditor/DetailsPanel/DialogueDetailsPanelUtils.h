...
public:
	...
	/**
	 * Tries to get the attendee name of the struct by using that structs PropertyName
	 * 1. Tries to get the value from the AttendeeNamePropertyHandle of that struct.
	 * 2. Gets the AttendeeName from the Node that has this property.
	 */
	static FName GetAttendeeNameFromPropertyHandle(const TSharedRef<IPropertyHandle>& AttendeeNamePropertyHandle);

	/** Gets all the attendee names of the Dialogue sorted alphabetically */
	static TArray<FName> GetDialogueSortedAttendeeNames(UDlgDialogue* Dialogue);
};
