...
{
	if (!HasAttendeeName() || !Dialogue.IsValid() || !GetObject())
	{
		return false;
	}
...
UObject* FDialogueEnumTypeWithObject_CustomRowHelper::GetObject() const
{
	...
	const FName AttendeeName = GetAttendeeName();
	if (AttendeeName == NAME_None)
	{
		return nullptr;
	}

	UClass* Class = Dialogue->GetAttendeeClass(AttendeeName);
	if (!Class)
	{
		return nullptr;
	}
...
{
	return LOCTEXT("BrowseButtonToolTipText", "Browse Attendee Asset in Content Browser");
}
...
FText FDialogueEnumTypeWithObject_CustomRowHelper::GetJumpToObjectText() const
{
	...
	{
		return LOCTEXT("OpenObjectBlueprintTooltipKey", "Open Attendee Blueprint Editor");
	}

	// Native Class
	return FText::Format(
		LOCTEXT("OpenObjectBlueprintTooltipKey", "Open Attendee Source File in {0}"),
...
FName FDialogueEnumTypeWithObject_CustomRowHelper::GetAttendeeName() const
{
	if (!AttendeeNamePropertyHandle.IsValid())
	{
		return NAME_None;
	}

	return FDialogueDetailsPanelUtils::GetAttendeeNameFromPropertyHandle(AttendeeNamePropertyHandle.ToSharedRef());
}
