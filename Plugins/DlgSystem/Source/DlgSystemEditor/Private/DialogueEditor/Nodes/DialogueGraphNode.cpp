...
FText UDialogueGraphNode::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	...
	const FString FullString = DialogueNode->GetNodeAttendeeName().ToString();
...
FText UDialogueGraphNode::GetTooltipText() const
{
	...
	Args.Add(TEXT("AttendeeName"), DialogueNode ? FText::FromName(DialogueNode->GetNodeAttendeeName()) : FText::GetEmpty());
	...
	return FText::Format(LOCTEXT("GraphNodeTooltip", "AttendeeName = {AttendeeName}\nType = {NodeType}\n{Description}"), Args);
}
