	...
	if (FromPin)
	{
		if (UDialogueGraphNode* GraphNode = Cast<UDialogueGraphNode>(FromPin->GetOwningNode()))
		{
			if (GraphNode->IsRootNode())
			{
				...
				{
					DialogueNode->SetNodeAttendeeName(Dialogue->GetNodes()[0]->GetNodeAttendeeName());
				}
			}
			else
			{
				// Use the node from which we spawned this
				DialogueNode->SetNodeAttendeeName(GraphNode->GetDialogueNode().GetNodeAttendeeName());
			}
		}
	}
	else
